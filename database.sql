...
CREATE TABLE entradas (
  ...
  id_pelicula INT UNSIGNED NOT NULL,
  codigo_ticket CHAR(13) NOT NULL UNIQUE,
...
('smtp_usuario','emlvpel@gmail.com','Correo SMTP real','texto'),
...
SELECT p.id, p.titulo, p.fecha_funcion, p.hora_funcion, p.precio_entrada, p.capacidad, p.entradas_vendidas,
       COALESCE(r.reservadas, 0) AS entradas_reservadas,
       GREATEST(0, p.capacidad - COALESCE(r.reservadas, 0)) AS entradas_disponibles,
       ROUND((COALESCE(r.reservadas, 0) / p.capacidad) * 100, 2) AS porcentaje_ocupacion,
       COALESCE(SUM(CASE WHEN c.estado_pago='aprobado' AND c.estado='activo' THEN c.monto_total ELSE 0 END), 0) AS ingresos_aprobados,
       COUNT(DISTINCT CASE WHEN c.estado_pago='pendiente' AND c.comprobante_nombre IS NOT NULL THEN c.id END) AS compras_pendientes
FROM peliculas p
...
SELECT e.id, e.codigo_ticket, e.token_validacion, e.codigo_qr, e.numero_entrada, e.estado, cl.nombre_completo AS cliente,
